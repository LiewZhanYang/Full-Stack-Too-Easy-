...
class Announcement {
  ...
  static async getAllAnnouncementByID(id) {
    ...
    const sqlQuery = `SELECT * FROM Announcement WHERE AnnouncementID = ?`;
    const [result] = await connection.execute(sqlQuery, [id]);

    connection.end();

    if (result.length > 0) {
      const row = result[0]; // Only take the first row
      console.log("Database result:", row); // Log database result
      ...
      return null; // Return null if no announcement is found
    }
  }
  ...
  static async postAnnouncement(announcementDetails) {
    ...
    const values = [announcementDetails.Title, announcementDetails.Body];
