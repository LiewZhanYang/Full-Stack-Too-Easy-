...
const AdminPrograms = () => {
  const programs = [
    ...
    { id: 3, title: 'Future Entrepreneurs Labs', image: '/img/entrepreneurs.jpg' },
  ];
  ...
  return (
    <Container fluid className="admin-program-page p-4">
      ...
      <div className="admin-program-controls d-flex align-items-center my-3">
        ...
        />
        <Button variant="outline-secondary" className="admin-program-search-button me-2">
          <FaSearch />
        </Button>
        <Dropdown>
          <Dropdown.Toggle variant="outline-secondary" id="dropdown-basic" className="admin-program-filter">
            Type
          </Dropdown.Toggle>
      ...
      <Row className="admin-program-cards-row">
        {programs.map((program) => (
          <Col md={4} key={program.id} className="admin-program-card-col mb-4 d-flex align-items-stretch">
            <Card className="admin-program-card shadow-sm h-100">
              <div className="admin-program-card-image-container">
                <Card.Img variant="top" src={program.image} alt={program.title} className="admin-program-card-image" />
              </div>
              <Card.Body className="d-flex flex-column justify-content-between">
                <Card.Title className="admin-program-card-title">{program.title}</Card.Title>
                <Button variant="warning" className="admin-program-edit-button d-flex align-items-center">
